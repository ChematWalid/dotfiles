...
func loadTheme() Theme {
	...
	configPath := filepath.Join(home, ".config", "goful", "theme.json")
	data, err := os.ReadFile(configPath)
	if err != nil {
		_ = os.MkdirAll(filepath.Dir(configPath), 0755)
		if b, err := json.MarshalIndent(mochaDefaults, "", "  "); err == nil {
...
func setCatppuccinMocha(t Theme) {
	base := tcell.GetColor(t.Base)
	mantle := tcell.GetColor(t.Mantle)
	text := tcell.GetColor(t.Text)
	mauve := tcell.GetColor(t.Mauve)
	pink := tcell.GetColor(t.Pink)
	red := tcell.GetColor(t.Red)
	peach := tcell.GetColor(t.Peach)
	yellow := tcell.GetColor(t.Yellow)
	green := tcell.GetColor(t.Green)
	teal := tcell.GetColor(t.Teal)
	blue := tcell.GetColor(t.Blue)

	d := tcell.StyleDefault.Foreground(text).Background(base)
	look.SetDefault(d)
	look.SetTitle(d.Foreground(mauve).Background(mantle).Bold(true))
	look.SetDirectory(d.Foreground(blue).Bold(true))
	look.SetExecutable(d.Foreground(green).Bold(true))
	look.SetSymlink(d.Foreground(teal))
	look.SetSymlinkDir(d.Foreground(pink).Bold(true))
	look.SetMarked(d.Foreground(yellow).Bold(true))
	look.SetHighlight(d.Foreground(peach).Bold(true))

	look.SetFinder(tcell.StyleDefault.Foreground(base).Background(mauve).Bold(true))
	look.SetProgress(tcell.StyleDefault.Foreground(base).Background(mauve))

	look.SetPrompt(d.Foreground(mauve).Bold(true))
	look.SetCmdline(d)
	look.SetCmdlineCommand(d.Foreground(green).Bold(true))
	look.SetCmdlineMacro(d.Foreground(peach))
	look.SetCmdlineOption(d.Foreground(yellow))

	look.SetMessageInfo(d.Foreground(green).Bold(true))
	look.SetMessageError(d.Foreground(red).Bold(true))
}
...
func config(g *app.Goful, is_tmux bool) {
	...
	menu.Add("look",
		"c", "catppuccin   ", func() { setCatppuccinMocha(loadTheme()) },
